...
1. 특정 채널을 리스너를 한다. 총 두개의 채널을 리스너 할 예정인데 다음과 같다.
	- statusMusician
	- statusAlbum
	
2. 각 채널은 다음과 같은 코드값을 통해서 각 액션에 해당하는 부분을 구독할 것이다.
	- 뮤지션이 새로 등록될 때
	- 뮤지션의 정보가 변경될 때
	- 앨범이 새로 등록될 때
	- 앨범의 정보가 변경될 때
...
뮤지션에 대한 statuscode : new
새로운 뮤지션이 등록되면 그 정보를 받아본다.
...
뮤지션에 대한 statuscode : update
뮤지션 정보가 변경이 되면 그 정보를 받아본다.
...
앨범에 대한 statuscode : new
신규 앨범이 등록되면 그 정보를 받아본다.
...
앨범에 대한 statuscode : update
앨범 정보가 변경되면 그 정보를 받아본다.
